"""
QeMLflow Core Utilities
====================

Common utilities and helper functions for QeMLflow.
...
def setup_logging(
    ...
    """
    Setup logging configuration for QeMLflow.
    ...
    logger = logging.getLogger("qemlflow")
    logger.info(f"QeMLflow logging initialized at {level} level")
    return logger
...
def print_environment_report(env_info: Optional[Dict[str, Any]] = None) -> Any:
    ...
    print("QeMLflow Environment Report")
    print("=" * 50)
...
def create_directory_structure(base_filepath: str) -> Any:
    """
    Create standard QeMLflow project directory structure.
    ...
    print(f"Created QeMLflow project structure in {base_filepath}")
...
def validate_input(data: Any, data_type: Optional[str] = None) -> bool:
    """
    Validate input data for QeMLflow operations.
    ...
    if data_type == "dict" and not isinstance(data, dict):
        ...
        return False
    elif data_type == "pandas" and not isinstance(data, pd.DataFrame):
        return False
