...
Provid    @classmethod
    def from_file(cls, config_path: str, use_cache: bool = True) -> 'ChemMLConfig':
        """Load configuration from YAML file with caching support."""
        if use_cache:
            config_dict = cached_yaml_load(config_path)
        ...
        return cls.from_dict(config_dict)ironment-based settings, feature flags, and integration controls.
"""
import os
...
class ChemMLConfig:
    ...
    experiment_tracking: ExperimentTrackingConfig = field(default_factory=
        ExperimentTrackingConfig)
    models: ModelConfig = field(default_factory=ModelConfig)
    preprocessing: PreprocessingConfig = field(default_factory=
        PreprocessingConfig)
    visualization: VisualizationConfig = field(default_factory=
        VisualizationConfig)
    ...
    def _apply_environment_overrides(self):
        ...
        self.debug_mode = os.getenv('CHEMML_DEBUG', str(self.debug_mode)
            ).lower() == 'true'
        ...
        self.cache_directory = os.getenv('CHEMML_CACHE_DIR', self.
            cache_directory)
        self.enable_gpu_acceleration = os.getenv('CHEMML_GPU', str(self.
            enable_gpu_acceleration)).lower() == 'true'
        self.enable_distributed_computing = os.getenv('CHEMML_DISTRIBUTED',
            str(self.enable_distributed_computing)).lower() == 'true'
    ...
    def _validate_configuration(self):
        ...
        for directory in [self.data_directory, self.cache_directory, self.
            visualization.plot_directory]:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) ->'ChemMLConfig':
        ...
        return cls(experiment_tracking=exp_tracking, models=models,
            preprocessing=preprocessing, visualization=visualization,
            quantum=quantum, notebooks=notebooks, **config_dict)
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        def convert_value(value):
            ...
            if isinstance(value, tuple):
                ...
                return {k: convert_value(v) for k, v in dataclasses.asdict(
                    value).items()}
            return value
    ...
    def get_environment_summary(self) ->Dict[str, Any]:
        ...
        return {'timestamp': datetime.now().isoformat(), 'environment':
            self.environment, 'debug_mode': self.debug_mode,
            'python_version': sys.version, 'platform': platform.platform(),
            'chemml_version': chemml_version, 'experiment_tracking_enabled':
            self.experiment_tracking.enabled, 'gpu_acceleration_enabled':
            self.enable_gpu_acceleration, 'quantum_enabled': self.quantum.
            enabled, 'data_directory': os.path.abspath(self.data_directory),
...
def load_config() ->ChemMLConfig:
    ...
    config_paths = [os.getenv('CHEMML_CONFIG'), 'chemml_config.yaml',
        'config/chemml_config.yaml', os.path.expanduser(
        '~/.chemml/config.yaml')]
