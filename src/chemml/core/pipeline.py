...
class ChemMLPipeline:
    ...
    def __init__(
        ...
        self.experiment_name = (
            experiment_name
            or f"chemml_experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )

        # Setup caching
        self.cache_dir = (
            Path(cache_dir) if cache_dir else Path.home() / ".chemml" / "pipelines"
        )
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        # Components
        self.data_loader = ChemMLDataLoader()
    ...
    def add_default_models(self) -> "ChemMLPipeline":
    ...
    @classmethod
    def load_pipeline(cls, filepath: Union[str, Path]) -> "ChemMLPipeline":
    ...
    def run_full_pipeline(
        ...
        Returns:
            DataFrame with model comparison results
        """
        print("🚀 Running full ChemML pipeline...")
...
def create_pipeline(task_type: str = "regression", **kwargs) -> ChemMLPipeline:
    """Create a new ChemML pipeline."""
    return ChemMLPipeline(task_type=task_type, **kwargs)
