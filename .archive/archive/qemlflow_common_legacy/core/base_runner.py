"""
Base Runner Classes for QeMLflow Scripts
=====================================

Provides abstract base classes for creating modular, testable QeMLflow scripts
...
from ..config.environment import QeMLflowConfig


@dataclass
class SectionResult:
    """
    Result of executing a single section of a QeMLflow script.
...
class SectionRunner(ABC):
    ...
    Each section of a QeMLflow script should inherit from this class,
    implementing the execute method with specific logic.
    """

    def __init__(self, config: QeMLflowConfig, section_name: str):
        """
        Initialize section runner.

        Args:
            config: QeMLflow configuration
...
class BaseRunner(ABC):
    ...
    def execute_section(self, section_name: str) -> SectionResult:
        ...
        if result.success:
            ...
            if not self.config.force_continue:
                ...
                self.logger.error(
                    "Set QEMLFLOW_FORCE_CONTINUE=true to continue on errors"
                )
                raise RuntimeError(
    ...
    def generate_report(self) -> None:
        ...
        with open(report_file, "w") as f:
            f.write(f"QeMLflow Execution Report: {summary.script_name}\n")
