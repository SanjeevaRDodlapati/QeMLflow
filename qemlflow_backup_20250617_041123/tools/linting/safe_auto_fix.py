...
class SafeLintingAutoFix:
    ...
    def __init__(self, root_path: Optional[Path] = None, backup_dir: Optional[Path] = None):
        self.root = root_path or Path.cwd()
        self.backup_dir = backup_dir or self.root / "backups" / f"auto_fix_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        while i < len(lines):
            ...
            if stripped.startswith('return ') and not self._is_inside_function(lines, i):
                ...
                if func_line_idx is not None:
                    ...
                    fixed_lines.append(f"# TODO: Fix orphaned return statement: {stripped}")
    ...
    def _find_preceding_function(self, lines: List[str], line_idx: int) -> Optional[int]:
    ...
    def fix_file_safely(self, file_path: Path, dry_run: bool = True) -> FixResult:
        ...
        try:
            ...
            if not syntax_valid:
                current_content, changes = self.fix_return_outside_function(current_content)
            ...
            if not dry_run:
                ...
                if not final_syntax_valid:
                    result.errors.append(f"Final syntax validation failed: {final_error}")
                    # Restore backup
                    if result.backup_path:
                        self.restore_from_backup(file_path)
                    return result
    ...
    def run_safe_auto_fix(self, dry_run: bool = True, max_files: Optional[int] = None) -> SafetyReport:
        ...
        for i, file_path in enumerate(files_with_issues):
            print(f"🔍 Processing {i+1}/{len(files_with_issues)}: {file_path.relative_to(self.root)}")
...
def main():
    ...
    parser.add_argument("--live", action="store_true", 
                       help="Run in live mode (actually apply fixes)")
    parser.add_argument("--max-files", type=int, 
                       help="Maximum number of files to process")
    parser.add_argument("--test-mode", action="store_true",
                       help="Run self-tests first")
