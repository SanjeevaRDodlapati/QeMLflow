...
class QeMLflowRenamer:
    ...
    def find_files_to_process(self):
        ...
        for root, dirs, files in os.walk('.'):
            ...
            for file in files:
                ...
                if (not self.should_skip_path(file_path) and 
    ...
    def dry_run(self):
        ...
        print(f"   • Directory renames: {len([c for c in self.changes_log if c.get('type') == 'directory_rename'])}")
        print(f"   • Total changes: {sum(len(c.get('changes', [])) for c in self.changes_log)}")
    ...
    def execute_rename(self):
        ...
        try:
            ...
            with open(log_file, 'w') as f:
                json.dump({
                    ...
                    'summary': {
                        ...
                        'directories_renamed': len([c for c in self.changes_log if c.get('type') == 'directory_rename'])
                    }
                }, f, indent=2)
            ...
            print(f"🔄 You can rollback using: python {sys.argv[0]} --rollback {self.backup_dir}")
            return False
    ...
    def rollback(self, backup_dir):
        ...
        try:
            ...
            current_backup = f"pre_rollback_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def main():
    ...
    parser.add_argument('--backup-only', action='store_true',
                       help='Create backup only, do not rename')
    parser.add_argument('--execute', action='store_true',
                       help='Execute the renaming process')
