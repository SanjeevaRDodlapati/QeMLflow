    ...
    def test_file_detection(self):
        ...
        try:
            ...
            script_path = os.path.join(
                self.original_dir, "tools/migration/safe_rename_to_qemlflow.py"
            )
            ...
            if result.returncode == 0:
                ...
                self.log_test(
                    "File Detection", False, f"Script failed: {result.stderr}"
                )
    ...
    def test_backup_creation(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                backup_dirs = [
                    d for d in os.listdir(".") if d.startswith("chemml_backup_")
                ]
                ...
                if backup_created:
                    ...
                    self.log_test(
                        "Backup Creation", False, "No backup directory created"
                    )
            else:
                self.log_test(
                    "Backup Creation", False, f"Backup failed: {result.stderr}"
                )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if changes_made:
                    # Now test rollback
                    backup_dirs = [
                        d for d in os.listdir(".") if d.startswith("qemlflow_backup_")
                    ]
                    if backup_dirs:
                        backup_dir = backup_dirs[0]
                        ...
                        if rollback_result.returncode == 0:
                            ...
                            rollback_successful = (
                                "ChemML" in content_restored
                                and "QeMLflow" not in content_restored
                            )
                        ...
                        self.log_test(
                            "Rollback Functionality", False, "No backup directory found"
                        )
                else:
                    self.log_test(
                        "Rollback Functionality",
                        False,
                        "No changes were made initially",
                    )
            else:
                self.log_test(
                    "Rollback Functionality",
                    False,
                    f"Initial execution failed: {result.stderr}",
                )
    ...
    def test_directory_structure_preservation(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                expected_changes = {"src/chemml": "src/qemlflow"}
                ...
                self.log_test(
                    "Directory Structure", False, f"Script failed: {result.stderr}"
                )
