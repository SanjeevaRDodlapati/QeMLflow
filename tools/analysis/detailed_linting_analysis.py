...
def print_detailed_analysis(module_stats, error_stats, file_count, total_lines):
    ...
    total_files_with_issues = len(set().union(*[stats['files_with_issues'] for stats in module_stats.values()]))
    ...
    print(f"{'Module':<25} {'Issues':<8} {'Files':<6} {'Syntax':<7} {'Import':<7} {'Complex':<8} {'Undef':<6}")
    print("-" * 80)
    
    # Sort modules by total issues
    sorted_modules = sorted(module_stats.items(), key=lambda x: x[1]['total_issues'], reverse=True)
    
    for module, stats in sorted_modules:
        if stats['total_issues'] == 0:
            continue
            
        print(f"{module:<25} {stats['total_issues']:<8} {len(stats['files_with_issues']):<6} "
    ...
    complexity_count = sum(stats['complexity_issues'] for stats in module_stats.values())
