        ...
        try:
            ...
            for directory in directories:
                ...
                if result.returncode == 0:
                    ...
                    print(f"   Warning: autoflake failed for {directory}: {result.stderr}")
        ...
        try:
            ...
            for directory in directories:
                ...
                result = subprocess.run([
                    ...
                    "--exclude", r"/(\.git|__pycache__|\.pytest_cache|build|site)/",
                    str(dir_path),
                ], capture_output=True, text=True, cwd=self.base_dir)
                
                if result.returncode == 0:
                    # Count reformatted files
                    reformatted = len([line for line in result.stderr.split('\n') 
    ...
    def safe_organize_imports(self) -> int:
        ...
        try:
            ...
            for directory in directories:
                ...
                if result.returncode == 0:
                    # Count fixed files
                    fixed = len([line for line in result.stderr.split('\n') 
                               if 'Fixing' in line])
                    count += fixed
                else:
    ...
    def safe_remove_whitespace(self) -> int:
        ...
        for pattern in patterns:
            for file_path in self.base_dir.glob(pattern):
                if any(skip in str(file_path) for skip in ['__pycache__', '.git', 'build']):
                    continue
                ...
                try:
                    ...
                    if file_path.suffix == '.py':
                        ...
                        try:
                            ...
                            print(f"   Warning: Skipping {file_path} due to syntax error")
                            continue
...
def main():
    parser = argparse.ArgumentParser(description='ChemML Safe Quick Wins Implementation')
