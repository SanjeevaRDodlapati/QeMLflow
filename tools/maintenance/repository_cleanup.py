...
class RepositoryCleanup:
    ...
    def consolidate_validation_outputs(self):
        ...
        for file_path in validation_files:
            if "migration_validation" not in str(
                file_path
            ):  # Don't move files already in target
                ...
                try:
                    ...
                    self.log_action(
                        "CONSOLIDATE",
                        f"{relative_path} -> {new_path.relative_to(self.root_path)}",
                    )
        ...
        self.log_action(
            "CONSOLIDATE",
            f"Moved {consolidated_count} validation files to reports/migration_validation/",
        )
        ...
        for tool_path_str in migration_tools:
            ...
            if tool_path.exists():
                ...
                try:
                    ...
                    self.log_action(
                        "ARCHIVE",
                        f"{tool_path_str} -> {archive_file.relative_to(self.root_path)}",
                    )
                except Exception as e:
    ...
    def clean_temporary_files(self):
            ...
            for item in self.root_path.glob(pattern):
                try:
                    if item.is_file():
                        ...
                        cleaned_size += size
                        self.log_action(
                            "DELETE", f"Temp file: {item.relative_to(self.root_path)}"
                        )
                    elif item.is_dir():
                        ...
                        self.log_action(
                            "DELETE",
                            f"Temp directory: {item.relative_to(self.root_path)}",
                        )
                except Exception as e:
        ...
        self.log_action(
            "CLEAN",
            f"Removed {cleaned_count} temp items, freed {cleaned_size/(1024*1024):.1f} MB",
        )
    ...
    def remove_redundant_validation_scripts(self):
            ...
            try:
                ...
                if files:
                    ...
                    for file_path in files:
                        if file_path.is_file():
                            shutil.move(
                                str(file_path), str(target_dir / file_path.name)
                            )
                    self.log_action(
                        "MOVE",
                        f"ML outputs moved to reports/migration_validation/ml_outputs/",
                    )
        ...
        self.log_action(
            "REMOVE", f"Removed {removed_count} redundant validation scripts"
        )
        ...
        for doc_path_str in key_docs:
            ...
            if doc_path.exists():
                ...
                try:
                    ...
                    self.log_action(
                        "ORGANIZE",
                        f"Doc: {doc_path_str} -> {target_path.relative_to(self.root_path)}",
                    )
                except Exception as e:
                    print(f"⚠️  Could not organize {doc_path}: {e}")

        self.log_action(
            "ORGANIZE", f"Organized {organized_count} key documentation files"
        )
    ...
    def create_cleanup_summary(self):
        ...
        summary_file = (
            self.root_path / "reports" / "migration_validation" / "cleanup_summary.json"
        )
        with open(summary_file, "w") as f:
            json.dump(summary, f, indent=2)

        self.log_action(
            "CREATE", f"Cleanup summary: {summary_file.relative_to(self.root_path)}"
        )
