...
def main():
    ...
    if backups_path.exists():
        for item in backups_path.iterdir():
            if item.is_dir() and ("lint" in item.name.lower() or "robust" in item.name.lower()):
    ...
    try:
        ...
        size_saved = cleanup_summary["size_before_mb"] - cleanup_summary["size_after_mb"]
    ...
    summary_file = repo_root / "reports" / "migration_validation" / "final_cleanup_summary.json"
