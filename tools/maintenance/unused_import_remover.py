...
class UnusedImportRemover:
    ...
    def __init__(self, project_root: str):
        ...
        self.backup_dir = self.project_root / "backups" / f"unused_imports_{int(time.time())}"
        ...
        self.preserve_imports = {
            ...
            'torch', 'tensorflow', 'sklearn', 'rdkit',
            # ChemML core imports
            'chemml', 'ChemMLError',
    ...
    def get_f401_errors(self) -> List[Tuple[str, int, str]]:
        ...
        try:
            cmd = ['python', '-m', 'flake8', '--select=F401', '--format=%(path)s:%(row)d:%(col)d:%(text)s', 'src/']
    ...
    def is_safe_to_remove(self, import_name: str, file_path: str, line_content: str) -> bool:
        ...
        framework_imports = ['sklearn', 'torch', 'tensorflow', 'rdkit', 'numpy', 'pandas']
...
def main():
    ...
    print(f"✅ Success rate: {(removed/total*100):.1f}%" if total > 0 else "✅ No errors to fix")
