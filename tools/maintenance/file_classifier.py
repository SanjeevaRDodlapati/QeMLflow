...
class QeMLflowFileClassifier:
    ...
    def _matches_pattern(self, filepath: str, pattern: str) -> bool:
        ...
        if pattern.endswith("/**/*"):
            ...
            if fnmatch.fnmatch(filepath, dir_pattern) or filepath.startswith(
                dir_pattern + "/"
            ):
                return True
    ...
    def analyze_repository(self) -> Dict[str, List[str]]:
        ...
        classification = {"core": [], "middle": [], "outer": []}

        for root, dirs, files in os.walk(self.repo_root):
            # Skip hidden directories except .config
            dirs[:] = [
                d for d in dirs if not d.startswith(".") or d in [".config", ".github"]
            ]
    ...
    def apply_permissions(
        self, classification: Dict[str, List[str]], dry_run: bool = True
    ):
                ...
                if current_permissions != target_permissions:
                    if not dry_run:
                        try:
                            ...
                            changes_made.append(
                                f"Changed {file_path}: {oct(current_permissions)} -> {oct(target_permissions)}"
                            )
                        except Exception as e:
                            print(f"Error changing permissions for {file_path}: {e}")
                    else:
                        changes_made.append(
                            f"Would change {file_path}: {oct(current_permissions)} -> {oct(target_permissions)}"
                        )
...
"""
        ...
        for category, files in categories.items():
            if files:
                ...
                for file_path in files:
                    content += (
                        f"- `{file_path}` - {self._get_file_description(file_path)}\n"
                    )
...
"""
    ...
    def create_protection_config(self):
        ...
        config = {
            ...
            "emergency": {
                ...
                "emergency_contacts": ["qemlflow-emergency@example.com"],
    ...
    def audit_permissions(self) -> Dict[str, List[str]]:
        ...
        permission_map = {"core": 0o444, "middle": 0o644, "outer": 0o664}
    ...
    def main(self):
        ...
        parser = argparse.ArgumentParser(
            description="QeMLflow File Classification and Protection Tool"
        )
        parser.add_argument(
            "--analyze",
            action="store_true",
            help="Analyze repository and show classification",
        )
        parser.add_argument(
            "--protect", action="store_true", help="Apply protection permissions"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--audit", action="store_true", help="Audit current permissions"
        )
        parser.add_argument(
            "--setup", action="store_true", help="Setup protection system"
        )
        ...
        if args.setup:
            ...
            if issues["incorrect_permissions"]:
                ...
                for issue in issues["incorrect_permissions"][:10]:
                    print(
                        f"  {issue['file']}: {issue['current']} -> {issue['expected']} ({issue['level']})"
                    )
