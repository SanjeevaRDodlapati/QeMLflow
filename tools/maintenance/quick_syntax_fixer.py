...
def fix_unterminated_docstrings(file_path):
    ...
    try:
        ...
        if content.startswith("Sample data generators") or content.startswith(
            "Integration tests"
        ):
            content = '"""\n' + content
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                ...
                and line.strip()
                and not line.strip().startswith("__")
            ):
                lines[i] = '"""\n' + line
                modified = True
...
def quick_syntax_fix():
    ...
    for file_path in syntax_error_files:
        ...
        if is_valid_after:
            ...
            print(
                f"  ❌ Still broken: {error_after[:50] if error_after else 'Unknown'}..."
            )
            failed_count += 1
