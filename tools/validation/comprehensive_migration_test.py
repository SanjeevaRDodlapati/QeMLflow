...
class MigrationValidator:
    ...
    def test_no_chemml_references(self) -> bool:
        ...
        ignore_dirs = {
            ...
            ".pytest_cache",
            "node_modules",
            "qemlflow_backup_20250617_041123",
            "backups",
            ".venv",
            "venv",
            "qemlflow_env",
                ...
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        if re.search(r"\bchemml\b", line, re.IGNORECASE):
                            ...
                            if not re.search(
                                r"(migration|rename|old name|previous|backup)",
                                line,
                                re.IGNORECASE,
                            ):
                                chemml_refs.append(
        ...
        if chemml_refs:
            self.log_error(
                test_name, f"Found {len(chemml_refs)} remaining 'chemml' references:"
            )
            for ref in chemml_refs[:10]:  # Show first 10
                self.log_error(
                    test_name, f"  {ref['file']}:{ref['line']} - {ref['content']}"
                )
    ...
    def test_import_qemlflow(self) -> bool:
        ...
        try:
            ...
            self.log_success(
                test_name,
                f"QeMLflow imported successfully, version: {qemlflow.__version__}",
            )
        ...
        if setup_py.exists():
            with open(setup_py, "r") as f:
                ...
                if "qemlflow" in content.lower():
                    ...
                    config_tests.append(
                        ("setup.py", False, "Missing 'qemlflow' reference")
                    )
        ...
        if pyproject_toml.exists():
            with open(pyproject_toml, "r") as f:
                ...
                if "qemlflow" in content.lower():
                    ...
                    config_tests.append(
                        ("pyproject.toml", False, "Missing 'qemlflow' reference")
                    )
        ...
        if readme_md.exists():
            with open(readme_md, "r") as f:
                ...
                if "qemlflow" in content.lower():
                    ...
                    config_tests.append(
                        ("README.md", False, "Missing 'QeMLflow' reference")
                    )
    ...
    def test_python_syntax(self) -> bool:
        ...
        for py_file in python_files:
            ...
            try:
                ...
                self.log_warning(
                    test_name,
                    f"Could not parse {py_file.relative_to(self.root_path)}: {e}",
                )
        ...
        if syntax_errors:
            ...
            for error in syntax_errors:
                self.log_error(
                    test_name, f"  {error['file']}:{error['line']} - {error['error']}"
                )
            return False

        self.log_success(
            test_name, f"All {len(python_files)} Python files have valid syntax"
        )
        return True
    ...
    def test_git_status(self) -> bool:
        ...
        try:
            ...
            if result.stdout.strip():
                self.log_warning(
                    test_name, f"Working directory not clean:\n{result.stdout}"
                )
                return False
    ...
    def test_documentation_links(self) -> bool:
        ...
        doc_files = list(self.root_path.rglob("*.md")) + list(
            self.root_path.rglob("*.rst")
        )
        broken_links = []
        ...
        for doc_file in doc_files:
            ...
            try:
                ...
                self.log_warning(
                    test_name,
                    f"Could not check {doc_file.relative_to(self.root_path)}: {e}",
                )

        if broken_links:
            self.log_warning(
                test_name,
                f"Found {len(broken_links)} potentially broken internal links:",
            )
    ...
    def test_environment_activation(self) -> bool:
        ...
        if not env_path.exists():
            self.log_warning(
                test_name, "Virtual environment not found - creating new one"
            )
            try:
                subprocess.run(
        ...
        try:
            ...
            result = subprocess.run(
                [
                    str(python_exe),
                    "-c",
                    'import qemlflow; print(f"QeMLflow {qemlflow.__version__}")',
                ],
                cwd=self.root_path,
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                self.log_success(
                    test_name, f"Virtual environment working: {result.stdout.strip()}"
                )
                return True
            else:
                self.log_warning(
                    test_name,
                    f"QeMLflow not installed in virtual environment: {result.stderr}",
                )
                # Try to install it
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    self.log_info(
                        test_name, "Successfully installed QeMLflow in editable mode"
                    )
                    return True
                else:
                    self.log_error(
                        test_name, f"Failed to install QeMLflow: {result.stderr}"
                    )
                    return False
    ...
    def test_backup_integrity(self) -> bool:
        ...
        expected_backup_items = ["src/chemml", "chemml_env", "setup.py", "README.md"]
    ...
    def test_performance_basic(self) -> bool:
        ...
        try:
            ...
            if load_time > 5.0:
                self.log_warning(
                    test_name, f"Import time seems slow: {load_time:.2f} seconds"
                )
            else:
                self.log_success(
                    test_name, f"Import performance good: {load_time:.2f} seconds"
                )
    ...
    def run_all_tests(self):
        ...
        report_file = (
            self.root_path / "tools" / "validation" / "migration_validation_report.json"
        )
        ...
        if report["summary"]["failed"] == 0:
            ...
            print(
                "✅ The ChemML to QeMLflow migration appears to be complete and successful."
            )
        else:
            print(
            ...
            print(
                "❌ Please review and fix the issues before considering the migration complete."
            )
