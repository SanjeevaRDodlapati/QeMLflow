...
class MigrationFixer:
                ...
                try:
                    ...
                    if 'chemml' in content.lower() and content != original_content:
                        with open(py_file, 'w', encoding='utf-8') as f:
                            f.write(content)
                        files_fixed += 1
                        self.log_fix("Archive", f"Updated {py_file.relative_to(self.root_path)}")
    ...
    def fix_file_syntax(self, file_path: Path):
        ...
        for i, line in enumerate(lines):
            ...
            if i > 0 and line.strip() and not line[0].isspace() and lines[i-1].strip().endswith(':'):
                if line.strip() and not line.strip().startswith(('def ', 'class ', 'if ', 'for ', 'while ', 'try:', 'except', 'finally:', 'else:', 'elif ')):
                    line = '    ' + line
        ...
        if content != original_content:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)
            self.log_fix("Syntax", f"Fixed syntax in {file_path.relative_to(self.root_path)}")
        ...
        for file_path_str in broken_files:
            ...
            if file_path.exists():
                try:
                    ...
                    backup_dir = self.root_path / "tools" / "migration" / "broken_files_backup"
                    ...
                    files_removed += 1
                    self.log_fix("Cleanup", f"Moved broken file {file_path.relative_to(self.root_path)} to backup")
                except Exception as e:
    ...
    def fix_missing_imports(self):
        ...
        try:
            ...
            if 'from typing import' not in content and 'Optional' in content:
                ...
                lines.insert(insert_pos, 'from typing import Optional, Dict, Any, List, Union')
                content = '\n'.join(lines)
