...
class RobustLintingFramework:
    ...
    def __init__(self, root_path: Optional[Path] = None):
        ...
        self.backup_dir = self.root / "backups" / f"robust_lint_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def _check_tool_availability(self):
        ...
        potential_tools = [
            "flake8", "black", "isort", "mypy", "pylint", "bandit", 
            "autopep8", "yapf", "pytype", "pyre", "radon", "xenon",
            "safety", "semgrep", "pycodestyle"
        ]
    ...
    def run_tool_safely(self, tool_name: str, file_paths: List[Path], 
                       extra_args: List[str] = None) -> ToolResult:
    ...
    def _parse_tool_output(self, tool_name: str, stdout: str, stderr: str) -> List[LintingIssue]:
                ...
                if stdout.strip().startswith('[') or stdout.strip().startswith('{'):
                    ...
                    for item in data:
                        issues.append(LintingIssue(
                            ...
                            severity=self._pylint_type_to_severity(item.get('type', 'warning')),
                            tool=tool_name,
                            confidence=0.8,
                            detected_by=[tool_name]
                        ))
                ...
                if stdout.strip():
                    ...
                    for result in data.get('results', []):
                        issues.append(LintingIssue(
                            ...
                            tool=tool_name,
                            confidence=float(result.get('issue_confidence', 'MEDIUM').replace('HIGH', '0.9').replace('MEDIUM', '0.7').replace('LOW', '0.5')),
                            detected_by=[tool_name]
                        ))
            except json.JSONDecodeError:
                pass
    ...
    def find_consensus_issues(self, tool_results: List[ToolResult]) -> Dict[str, List[LintingIssue]]:
        ...
        for issue in all_issues:
            ...
            key = (issue.file_path, issue.line_number // 5 * 5, issue.rule_code[:1])  # Group by file, line range, and rule type
    ...
    def _merge_issues(self, issues: List[LintingIssue]) -> LintingIssue:
        ...
        return LintingIssue(
            ...
            confidence=min(1.0, 0.6 + 0.15 * len(all_tools)),  # Higher confidence with more tools
    ...
    def run_comprehensive_analysis(self, file_paths: List[Path]) -> ConsensusReport:
        ...
        # Run available tools
        available_linters = [tool for tool in ["flake8", "black", "isort", "mypy", "pylint", "bandit"] 
                           if self.available_tools.get(tool, False)]
        
        for tool in available_linters:
            print(f"  Running {tool}...")
            if tool == "flake8":
                result = self.run_tool_safely(tool, file_paths, ["--format=%(path)s:%(row)d:%(col)d: %(code)s %(text)s"])
            elif tool == "black":
                result = self.run_tool_safely(tool, file_paths, ["--check", "--diff"])
            elif tool == "isort":
                result = self.run_tool_safely(tool, file_paths, ["--check-only", "--diff"])
            elif tool == "mypy":
                result = self.run_tool_safely(tool, file_paths)
            elif tool == "pylint":
                result = self.run_tool_safely(tool, file_paths, ["--output-format=json"])
            elif tool == "bandit":
        ...
        actual_agreements = sum(len(group) for group in consensus_issues.values() if len(group) > 1)
        ...
        total_weighted_issues = strong_consensus_weight + moderate_consensus_weight + single_tool_weight
        ...
        report = ConsensusReport(
            ...
            consensus_issues=len(consensus_issues['strong_consensus']) + len(consensus_issues['moderate_consensus']),
    ...
    def generate_consensus_report(self, report: ConsensusReport) -> str:
        ...
        lines.append(f"  Strong Consensus (3+ tools): {len(report.issues_by_consensus.get('strong_consensus', []))}")
        lines.append(f"  Moderate Consensus (2 tools): {len(report.issues_by_consensus.get('moderate_consensus', []))}")
        lines.append(f"  Single Tool Detection: {len(report.issues_by_consensus.get('single_tool', []))}")
        ...
        if strong_consensus:
            ...
            for issue in strong_consensus[:10]:  # Show top 10
                lines.append(f"  {issue.file_path}:{issue.line_number} - {issue.rule_code}: {issue.message}")
                lines.append(f"    Detected by: {', '.join(issue.detected_by)} (confidence: {issue.confidence:.1%})")
        ...
        if report.reliability_score < 0.8:
            lines.append("  ⚠️ Low tool reliability - consider installing missing linters")
        if len(strong_consensus) > 0:
            lines.append(f"  🔧 Address {len(strong_consensus)} high-confidence issues first")
        if len(report.issues_by_consensus.get('single_tool', [])) > len(strong_consensus) * 2:
...
def main():
    ...
    parser.add_argument("--scan-project", action="store_true", help="Scan entire project")
