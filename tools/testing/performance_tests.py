...
def test_feature_extraction_performance():
    ...
    try:
        ...
        print(
            f"   ✅ Extracted features for {len(smiles_list)} molecules in {processing_time:.2f}s"
        )
...
def test_model_training_performance():
    ...
    try:
        ...
        for n_samples in [100, 500, 1000]:
            ...
            results.append(
                {"n_samples": n_samples, "linear_time": linear_time, "rf_time": rf_time}
            )
    ...
    for test_name, test_input in test_cases:
        try:
            ...
            if test_input is None:
                ...
                result = process_smiles(test_input)
                print(
                    f"      ✅ Handled gracefully: {len(result) if result else 0} results"
                )
                passed_tests += 1
