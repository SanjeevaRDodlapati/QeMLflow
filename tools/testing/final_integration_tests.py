...
def test_drug_discovery_workflow():
    ...
    try:
        ...
        print(f"   ✅ SMILES processing: {len(valid_molecules)}/{len(drug_molecules)} valid")
        ...
        if isinstance(features, list) and len(features) > 0:
            ...
            X = np.array([[f] if isinstance(f, (int, float)) else [len(str(f))] for f in features])
        else:
        ...
        if isinstance(features, list):
            X_test = np.array([[len(str(mol))] if mol else [0] for mol in test_processed])
        else:
        ...
        if isinstance(features, list) and len(features) > 1:
            ...
            for i in range(len(features)):
                for j in range(i+1, len(features)):
                    ...
                    sim = 1.0 / (1.0 + abs(hash(str(features[i])) - hash(str(features[j]))) % 100 / 100.0)
