...
class GitHubActionsMonitor:
    ...
    def _get_repo_info(self) -> Tuple[str, str]:
        ...
        try:
            ...
            raise ValueError(
                f"Could not parse GitHub repository from URL: {remote_url}"
            )
    ...
    def check_current_status(self) -> Dict[str, Any]:
        ...
        print(
            f"🔍 Checking GitHub Actions status for {self.repo_owner}/{self.repo_name}"
        )
            ...
            if all_root_causes:
                ...
                if most_common == "missing_imports":
                    recommendations.append(
                        "🎯 PRIORITY: Add missing typing imports (List, Dict, Optional, Union, Any)"
                    )
                    recommendations.append(
                        '🔧 Run: python -c "from typing import List, Dict, Optional, Union, Any"'
                    )
                    ...
                    recommendations.append(
                        "🎯 PRIORITY: Fix dependency installation issues"
                    )
                    recommendations.append(
                        "🔧 Update requirements.txt and package versions"
                    )
    ...
    def calculate_health_score(self) -> int:
        ...
        if "failure_analysis" in self.results:
            ...
            score -= min(
                failure_count * 10, 30
            )  # Deduct up to 30 points for recent failures
    ...
    def print_summary(self):
        ...
        if "workflow_status" in self.results:
            ...
            if status.get("latest_run"):
                ...
                print(
                    f"   🕒 Latest: {latest['name']} - {latest.get('conclusion', 'unknown')}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor and analyze GitHub Actions workflows"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--check-logs", action="store_true", help="Analyze failure logs in detail"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Maximum number of failures to analyze",
    )
